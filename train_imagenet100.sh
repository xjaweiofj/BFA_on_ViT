...
############### Neural network ############################
{
python3 retrain.py --dataset ${dataset} --data_path /data1/imagenet-100/   \
    ...
    --manualSeed ${seed} \
    --optimizer ${optimizer}  | tee log_Adam/train_attack_${model}_epoch${epochs}_lr5e-5_${dataset}.log
} &
