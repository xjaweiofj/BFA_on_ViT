...
def int2bin(input, num_bits):
    ...
    though in storage, int is stored in 32-bit 2's compelemnt format, but we cannot directly access a integer's single bit
    that's why we need this int2bin and bin2int
    '''
    output = input.clone()

    output[input.lt(0)] = 2**num_bits + output[input.lt(0)] # lt = less than
    # if input<0, then convert it
    # 
...
def bin2int(input, num_bits):
    ...
    '''
    mask = 2**(num_bits - 1) - 1 # mask=127
    output = -(input & ~mask) + (input & mask)

    # eg: input = 255 = 1111 1111
    # output=-(1111 1111 & 1000 0000)+(1111 1111 & 0111 1111)=-1000 0000+0111 1111=-1
